...
route('/api/messages/conversation', function () use ($messageController, $userController) {
    $request = validateRequest('GET', 'JSON', 'Bearer', 'getConversation');
    if ($request) {
        $result = $userController->validateUser();
        $success = $result['success'];
        if ($success) {
            $user = $result['user'];
            $userId = $user->user_id;
            $result = $messageController->getConversation($userId);
        }
        $jsonResult = json_encode($result);
        header('Content-Type: application/json; charset=utf-8');
        echo $jsonResult;
